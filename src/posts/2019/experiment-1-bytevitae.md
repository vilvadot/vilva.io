---
title: "⚗️ Experiment #1: ByteVitae.com"
permalink: blog/experiment-1-bytevitae/
date: '2019-05-15'
tags: [product-building]
---

![⚗️ Experiment #1: ByteVitae.com](/images/2019/05/spacex-549326-unsplash.jpg)

.project-button{color:#fff;background:#000;border:none;border-radius:100px;padding:10px 20px;outline:0;box-shadow:none}.project-button:hover{cursor:pointer;background:#0a0a0a}.center-content{display:flex;justify-content:center}

[Go to project](https://bytevitae.com)

> _\* This is a live post.  I will update it as the product grows\*_

The stats
=========

State: **Unreleased!**

Release date: Soon™, June 019 🤞🏻

👥 Users………………………………..……0  
💸 Revenue…………………………………0€

![](/images/2019/05/ByteVitae_mockup-1.png)

The problem
-----------

A few years ago, when I was designing full time, a developer friend came to me asking for help with a job change. He was a great dev, but lacked any design skills, and needed some way to make a good looking Currículum. After seeing the result, a friend of his contacted me and offered money for me to do the same for him.

Fast forward to this year. I participated in a RUN with my mates at [Devscola](http://www.devscola.org/), and when it finished suddenly there where 20 people needing CVs to land their first job.

So the idea kept ringing in my head.

The goal
--------

A tool **for developers** to create and keep an up to date, great looking CV in a fast and easy way.

Updates
-------

[🗓 Week #1 - Islands, Auth and Lies](https://blog.vilva.io/week-1-auth-islands-and-lies/)

[🗓 Week #2 - There is no journey without a ship!](https://blog.vilva.io/week-2-there-is-no-journey-without-a-ship-journey-bytevitae/)

[🗓 Week #3 - Typeform results, Stripe and friends](https://blog.vilva.io/week-3/)

[🗓 Week #4 & #5 - Payments, new Illustration and Shame](https://blog.vilva.io/week-4-5-payments-new-illustration-and-shame/)

Vilva.